...
def scan_workspace_pages(session_id: str, workspace: str) -> List[Dict[str, Any]]:
    """
    Scan workspace for HTML files and extract page structure with anchors.
    Returns a list of pages with their sections/anchors.
    """
    pages = []

    print(f"[Sandbox:{session_id}] ========== SCANNING WORKSPACE ==========")
    print(f"[Sandbox:{session_id}] Workspace path: {workspace}")
    print(f"[Sandbox:{session_id}] Workspace exists: {os.path.exists(workspace)}")
    print(f"[Sandbox:{session_id}] Workspace is dir: {os.path.isdir(workspace)}")

    # Use os.walk for robust file discovery
    html_files = []
    try:
        for root, dirs, files in os.walk(workspace):
            print(f"[Sandbox:{session_id}] Walking: {root}, dirs: {dirs}, files: {files}")
            for file in files:
                if file.endswith('.html') or file.endswith('.htm'):
                    full_path = os.path.join(root, file)
                    html_files.append(full_path)
                    print(f"[Sandbox:{session_id}] Found HTML file: {full_path}")
    except Exception as e:
        print(f"[Sandbox:{session_id}] Error walking workspace: {e}")

    # Also check directly with listdir as backup
    try:
        direct_files = os.listdir(workspace)
        print(f"[Sandbox:{session_id}] Direct listdir: {direct_files}")
        for f in direct_files:
            full_path = os.path.join(workspace, f)
            if (f.endswith('.html') or f.endswith('.htm')) and full_path not in html_files:
                html_files.append(full_path)
                print(f"[Sandbox:{session_id}] Added from listdir: {full_path}")
    except Exception as e:
        print(f"[Sandbox:{session_id}] Error with listdir: {e}")

    # Check if index.html specifically exists
    index_path = os.path.join(workspace, "index.html")
    print(f"[Sandbox:{session_id}] index.html path: {index_path}")
    print(f"[Sandbox:{session_id}] index.html exists: {os.path.exists(index_path)}")

    if os.path.exists(index_path) and index_path not in html_files:
        html_files.append(index_path)
        print(f"[Sandbox:{session_id}] Added index.html explicitly")

    print(f"[Sandbox:{session_id}] Total HTML files found: {len(html_files)}")
    print(f"[Sandbox:{session_id}] HTML files: {html_files}")
    print(f"[Sandbox:{session_id}] =========================================")
    ...
    for html_path in html_files:
        try:
            ...
            # Convert to URL path (index.html -> /, about.html -> /about.html)
            ...
            print(f"[Sandbox:{session_id}] Processing {rel_path}, content length: {len(content)}")

            # Extract page title
            title_match = re.search(r'<title[^>]*>([^<]+)</title>', content, re.IGNORECASE)
            title = title_match.group(1).strip() if title_match else rel_path

            # Extract sections - improved pattern matching
            sections = []
            seen_ids = set()

            # Pattern 1: Find all id attributes in section-like elements
            # Match: <section id="hero">, <div id="about">, etc.
            tag_with_id_pattern = r'<(section|div|article|nav|header|footer|main|aside|h[1-6])[^>]*\s+id\s*=\s*["\']([^"\']+)["\']'
            for match in re.finditer(tag_with_id_pattern, content, re.IGNORECASE):
                tag = match.group(1).lower()
                element_id = match.group(2)

                if element_id not in seen_ids:
                    seen_ids.add(element_id)
                    # Create readable title from id
                    readable_title = element_id.replace("-", " ").replace("_", " ").title()
                    sections.append({
                        "id": element_id,
                        "tag": tag,
                    ...
                    print(f"[Sandbox:{session_id}] Found section: #{element_id} ({tag})")

            # Pattern 2: Also find anchor links within the page (href="#...")
            anchor_links = re.findall(r'href\s*=\s*["\']#([^"\']+)["\']', content, re.IGNORECASE)
            for anchor in anchor_links:
                if anchor not in seen_ids:
                    seen_ids.add(anchor)
                    sections.append({
                        "id": anchor,
                        "tag": "anchor",
                        "title": anchor.replace("-", " ").replace("_", " ").title(),
                        "anchor": f"#{anchor}"
                    })
                    print(f"[Sandbox:{session_id}] Found anchor link: #{anchor}")

            # Find internal page links
            internal_links = []
            link_pattern = r'href\s*=\s*["\'](?!#|https?://|mailto:|tel:|javascript:)([^"\']+\.html?)["\']'
            for link_match in re.finditer(link_pattern, content, re.IGNORECASE):
                linked_page = link_match.group(1)
                if not linked_page.startswith('/'):
                    linked_page = '/' + linked_page
                if linked_page not in internal_links:
                    internal_links.append(linked_page)
            ...
            print(f"[Sandbox:{session_id}] Page: {url_path} - {title} ({len(sections)} sections)")

        except Exception as e:
            print(f"[Sandbox:{session_id}] Error scanning {html_path}: {e}")

    # Sort pages: index.html first, then alphabetically
    pages.sort(key=lambda p: (0 if p["path"] == "/" else 1, p["path"]))
...
The file must be saved as index.html in the current directory.""" if turn_number == 1 else prompt
        ...
        if dev_server.process:
            # Scan workspace for pages and sections
            pages = scan_workspace_pages(session_id, workspace)
        ...
        # Send pages update event
        pages = scan_workspace_pages(session_id, workspace)
        send_event("pages_updated", {"pages": pages})
        ...
        while True:
            try:
                ...
                if new_prompt:
                    ...
                    # Scan for updated pages after each turn
                    pages = scan_workspace_pages(session_id, workspace)
                    send_event("pages_updated", {"pages": pages})
