  ...
  useEffect(() => {
    ...
    const fetchSessionStatus = async () => {
      try {
        ...
        // Silently ignore fetch errors - WebSocket is the primary channel
        // Only log if we don't have a WebSocket connection yet
        if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
          console.log('[Session] Polling failed, will retry...', error instanceof Error ? error.message : '');
        }
      }
    };
    ...
    pollIntervalRef.current = setInterval(() => {
      // Skip polling if WebSocket is connected - no need for HTTP polling
