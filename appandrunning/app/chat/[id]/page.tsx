...
export default function ChatPage() {
  ...
  const [pages, setPages] = useState<Page[]>([]); // Available pages from backend
  ...
  useEffect(() => {
    ...
    if (latestEvent.event === "dev_server_started") {
      ...
      console.log('[Chat] dev_server_started event received, url:', url, 'data:', latestEvent.data);

      // Always try to extract pages
      const eventPages = latestEvent.data?.pages;
      if (eventPages && Array.isArray(eventPages)) {
        console.log('[Chat] Pages received from dev_server_started:', eventPages.length, eventPages);
        setPages(eventPages);
      } else {
        console.log('[Chat] No pages in dev_server_started event');
      }
    ...
    // Handle pages updated event
    if (latestEvent.event === "pages_updated") {
      const eventPages = latestEvent.data?.pages;
      console.log('[Chat] pages_updated event received:', latestEvent.data);
      if (eventPages && Array.isArray(eventPages)) {
        console.log('[Chat] Pages updated:', eventPages.length, eventPages);
        setPages(eventPages);
      } else {
        console.log('[Chat] No pages in pages_updated event');
      }
    }
        ...
        <div className={`flex flex-col ${devUrl ? 'w-1/2 border-r' : 'w-full'} transition-all`}>
          ...
          {devUrl && (
            <div className="w-1/2 flex flex-col">
              <WebsitePreview devUrl={devUrl} refreshTrigger={iframeRefreshTrigger} pages={pages} />
            </div>
          )}
        </div>
      </div>
